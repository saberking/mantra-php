<!DOCTYPE html>
<html>
<head>
	<?php $root_dir = $_SERVER["DOCUMENT_ROOT"] ?>
	<title>Mantra Recruitment Group</title>

	<?php include_once $root_dir . '/php/components/head.php' ?>

</head>
<body>

<div class="page-loader page-loader-visible">
</div>


	<?php include_once $root_dir . '/php/components/header-simple.php' ?>



	<section class="page-heading bg-dark-alpha-70 parallax-4 font-white" data-background-image="/img/fullwidth/fullwidth10.jpg">

		<div class="container mt-80">
			<div class="row">
				
				<div class="col-md-12 pt-30 pb-30">

					<h1 class="align-center">Contact Us</h1>


				</div>


			</div>
		</div>

	</section>

	<section class="page-section">

		<div class="container">


			<div class="row">

				<div class="col-md-8 col-sm-12 mb-sm-60">

				<?php include_once $root_dir . '/php/components/map.php'?>
				</div>

				<div class="col-md-4 col-sm-12">

					<div class="icon-box ib-icon-aside ib-icon-up">

						<div class="ib-icon">
							<i class="icon_mobile"></i>
						</div>

						<h5 class="ib-title">Phone</h5>

						<p class="ib-desc">
						<a href="tel:+443303802015"> 0330 380 2015</a>
						</p>

					</div> <!-- end .icon-box -->

					<div class="icon-box ib-icon-aside ib-icon-up">

						<div class="ib-icon">
							<i class="icon_mail_alt"></i>
						</div>

						<h5 class="ib-title">Email</h5>

						<p class="ib-desc">
						<a href="mailto:mantrarecruitmentgroup@gmail.com"> mantrarecruitmentgroup@gmail.com</a>
						</p>

					</div> <!-- end .icon-box -->

					<div class="icon-box ib-icon-aside ib-icon-up">

						<div class="ib-icon">
							<i class="icon_pin_alt"></i>
						</div>

						<h5 class="ib-title">Address</h5>

						<p class="ib-desc">
						Mantra Recruitment Group<br>
Epic House<br>
18 Darnall Road<br>
Sheffield<br>
S9 5AB 
						</p>

					</div> <!-- end .icon-box -->
					<div class="icon-box ib-icon-aside ib-icon-up">

						<div class="ib-icon">
						<i class="fa fa-whatsapp"></i>
						</div>

						<h5 class="ib-title">Whatsapp</h5>

						<p class="ib-desc">
						<a href="https://wa.me/447415703689">07415 703689</a>
						</p>

					</div> <!-- end .icon-box -->

					<div class="icon-box ib-icon-aside ib-icon-up">

						<div class="ib-icon">
						<a  class="social-icon si-facebook si-middle"></a>
						</div>

						<h5 class="ib-title">Facebook</h5>

						<p class="ib-desc">
						
						<a href="https://www.facebook.com/Mantrarecruitmentgroup/"> facebook.com/Mantrarecruitmentgroup</a>
						</p>

					</div> <!-- end .icon-box -->

				</div>

			</div> <!-- end .row -->

		</div> <!-- end .container -->

	</section> <!-- end .page-section -->



    <?php include_once $root_dir . '/php/components/footer.php' ?>


    <?php include_once $root_dir . '/php/components/scripts.php' ?>
</body>
</html>
